...
public class Parser {
    ...
    Primary generatePrimal(Stack<String> tata){

        while(!tata.isEmpty()){
            try{
            if(standartOperations.contains(tata.peek())){
                result.push(new Primary(tata.pop()));
                if(standartOperations.contains(tata.peek())) {
                    result.peek().setRight(generatePrimal(tata));
                }else
                    result.peek().setRight(new Primary(new MyInteger(Integer.parseInt(tata.pop()))));
                if(standartOperations.contains(tata.peek())) {
                    result.peek().setLeft(generatePrimal(tata));
                }else
                    result.peek().setLeft(new Primary(new MyInteger(Integer.parseInt(tata.pop()))));
                return result.pop();
            }else{
                return new Primary(new MyInteger(Integer.parseInt(tata.pop())));
            }
